  ...
  const handleAuth = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE || "http://localhost:8000/api"}${endpoint}`,
        {
          method: "POST",
      ...
      setRestaurantName(data.restaurant_name || auth.restaurant_name || null);
      if (!data.restaurant_name) {
        try {
          const me = await authApi.me();
          setRestaurantName(me.restaurant_name || null);
        } catch {
          // Ignore optional restaurant profile fetch failure.
        }
      }
      setIsAuthed(true);
      setError("");
